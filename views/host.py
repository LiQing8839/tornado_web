...
class HostTYHandler(BaseHandler):
    ...
    def get(self):
        ...
        self.render("host.html", web_title="运维管理平台", user=self.current_user, thead=thead, tbody=self.application.data.GetDAll(sql), page="通用主机", host="host_ty")
...
class HostTYACHandler(BaseHandler):
    ...
    def get(self,input):
        if input == 'add':
            ...
            self.render("host_action.html",web_title="运维管理平台",user=self.current_user, page="添加通用主机", group=self.application.data.GetAll(sql), action='host_ty')
        elif input == 'edit':
...
class HostWDHandler(BaseHandler):
    ...
    def get(self):
        ...
        self.render("host.html",web_title="运维管理平台",user=self.current_user,thead=thead,tbody=self.application.data.GetDAll(sql),page="万达主机",host="host_wd")

class HostWDACHandler(BaseHandler):
    @tornado.web.authenticated
    def get(self,input):
        if input == 'add':
            self.render("host_action.html",web_title="运维管理平台",user=self.current_user,page="添加万达主机",action="cinema_wd")
        elif input == 'edit':
            id = self.get_argument('id')
            sql = "select * from cinema_wd where id = %s"%id
            self.render("host_edit.html",web_title="运维管理平台",user=self.current_user,page="修改万达主机",data=self.application.data.GetOne(sql),group='')
        else:
            self.render("404.html",error="没有找到页面")
